...
if [[ "$OS" == "macOS" ]]; then
    ...
    if ! command -v tofu &> /dev/null; then
        ...
        # Install directly from Homebrew core
        brew install opentofu
        
        # Create terraform alias for compatibility
        if ! grep -q "alias terraform" ~/.zsh/aliases.zsh 2>/dev/null; then
        ...
        if command -v tofu &> /dev/null; then
            ...
            echo -e "${RED}OpenTofu installation failed. Please install manually.${NC}"
            echo -e "${YELLOW}Visit https://opentofu.org/docs/intro/install/ for installation guides.${NC}"
        fi
    else
